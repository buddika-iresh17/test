...
async function downloadSession() {
  if (!fs.existsSync(sessionFile)) {
    ...
    console.log('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Downloading session file from Mega...');
...
async function connectToWA() {
  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Disconnected with status code:', statusCode);

      if (statusCode !== DisconnectReason.loggedOut) {
        console.log('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Reconnecting...');
        await connectToWA();
      } else {
        console.log('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Session logged out. Delete session files and scan QR again ...');
        process.exit(0);
      }
    } else if (connection === 'open') {
      console.log("🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Bot connected successfully ✅ ...");
...
╚═════════════════════⫸`;
      ...
      try {
        ...
        console.error("🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Failed to send owner message:", error.message);
      }
    }
  });
  ...
  conn.ev.on('error', (err) => {
    console.error('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Connection error:', err);
  });
...
app.listen(port, () => {
  console.log(`🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Server running 🏃 on port ${port}`);
  ...
  setTimeout(async () => {
    try {
      ...
      console.error("🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Startup error:", e);
      process.exit(1);
    }
  }, 4000);
});